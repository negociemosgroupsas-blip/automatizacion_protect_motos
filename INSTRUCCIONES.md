...
- **Registrar un pago:** botón "Registrar pago" en la fila del cliente → completa cuota, valor, fecha y comprobante → Guardar. Esto agrega una fila nueva en "Pagos" (nunca borra ni sobrescribe nada).
- **Clientes que ya llevaban pagando antes de tener este sistema:** si escribes un número de cuota mayor al que el sistema esperaba (ej. esperaba la 2 y escribes la 5), el panel te pregunta si las cuotas intermedias (2, 3 y 4) fueron pagadas antes de tener el sistema. Si aceptas, las registra automáticamente como "Cubierta — pago histórico anterior al sistema" y luego registra tu pago real de la cuota 5. Si cancelas, no se registra nada y puedes decidir cobrarle lo atrasado primero.
